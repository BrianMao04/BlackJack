...
# Date: 02/05/2014
# File Name: Blackjack_Main_Classes01.py
# Description: Classes for making Blackjack game 
#########################################
...
class Card(object):
    ...
    def __init__(self,  suit=0,  rank=0):     #Card attributes
        ...
        self.rank  =  rank
        self.value =  0
        self.visible = True                  #Boolean for whether front side of the card is visable 
        self.image = None
    ...
    def update(self):
        ...
        if self.rank<=8:                     #Value of cards for the first 8 elements in the list (ones that are not face cards or an ace)
...
class Button(object):
    ...
    def draw(self, surface):
        # Draw a button as a rectangle with text inside
        if self.hovering==True:
            pygame.draw.rect(surface, self.hovercolour, self.rect, 4)
            self.font_surf = self.font.render(self.text, True, self.hovercolour) #Colour output if mouse is hovering over the button
        else:
            pygame.draw.rect(surface, self.defcolour, self.rect, 6)
